(function() {
 ...
 function getVersionInfo(name) {
  ...
  if (canDetectInternalAPI) {
    ...
    },
    {
      ...
      mapper: function (_opts) {
        ...
        if (!_opts.clientId) {
          opts.clientId = opts.bucket = 'shaitu';
        }
        return opts;
      },
      callbackMapper: function ({photoInfos=[]}){
        return {
          photoInfos: photoInfos.map(photo => mixin(photo, {picUrl: photo.picKey}))
        }
      }
    },
    {
  ...
  }].forEach(({name, businessName, moduleName, methodName, fallbackRegFn}) => {
    sentinel.increase();
    isInternalSupported(`${businessName}.${moduleName}.${methodName}`).then(supported => {
      sentinel.decrease();
      if (!supported) return fallbackRegFn();
      MTNB.reg({name, businessName, moduleName, methodName});
    });
  });
